...
let restServer = function(core, options, end) {
  ...
  let logger = core.getLogger('electron-beauty');
  ...
  server.pre(restify.bodyParser({ maxFieldsSize: 10485760, mapParams: false }));
  server.pre(restify.gzipResponse());
  _.forEach(config.uses, (use) => server.use(use));
  ...
  let funcErrorOccur = (req, res, route, error) => {
    if (!res.headersSent) {
  ...
  server.on('error', funcErrorOccur);
  server.on('uncaughtException', funcErrorOccur);
